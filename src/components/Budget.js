...
const Budget = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (e.target.value > budgetLimit) {
      window.alert(`Please enter a value less than or equal to the budget limit(£ ${budgetLimit}).`);
      setInputBudget('');
    } else if (e.target.value < totalExpenses) {
      window.alert(`Please enter a value more than or equal to the budget limit(£ ${totalExpenses}).`);
      setInputBudget('');
    } else {
      dispatch({
        type: 'SET_BUDGET',
